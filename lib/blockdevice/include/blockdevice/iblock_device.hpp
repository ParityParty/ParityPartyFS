...
/// @brief Represents a specific position on the disk defined by a block index and an offset.
...
/// @brief Abstract interface for block-level storage operations.
///
...
public:
    ...
    /// @brief Writes a sequence of bytes into the device at a specified data location.
    ///
    ...
    /// @brief Reads a sequence of bytes from the device at a specified data location.
    ///
    ...
    /// @brief Returns the physical (raw) block size of the underlying device.
    /// @return Size of one raw block in bytes.
    virtual size_t rawBlockSize() const = 0;

    /// @brief Returns the usable data size within a block.
    ///
