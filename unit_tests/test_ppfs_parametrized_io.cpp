...
TEST_P(PpFSParametrizedTest, WriteRead_SmallData)
{
    ...
    for (size_t i = 0; i < data_size; ++i) {
        ASSERT_EQ(read_data[i], write_data_vec[i])
            << "Read data doesn't match written data at index " << i << " for " << GetParam().test_name;
    }
...
TEST_P(PpFSParametrizedTest, WriteAppendRead)
{
    ...
    static_vector<uint8_t> write_data1(write_buf1.data(), write_buf1.size(), write_data1_vec.size());
    ...
    static_vector<uint8_t> write_data2(write_buf2.data(), write_buf2.size(), write_data2_vec.size());
