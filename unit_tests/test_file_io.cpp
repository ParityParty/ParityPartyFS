...
TEST(FileIO, Dupa)
{
    ...
    FsConfig options { .total_size = disk.size(),
        ...
        .use_journal = false };
    auto format_res = fs.format(options);
    ASSERT_TRUE(format_res.has_value());
    ...
    auto indexes_per_block = 128 / sizeof(block_index_t);
    std::vector<uint8_t> data(128
        * (12 + indexes_per_block + indexes_per_block * indexes_per_block
            + indexes_per_block * indexes_per_block * 10));

    for (size_t i = 0; i < data.size(); ++i)
        data[i] = uint8_t(i % 128);

    auto open_res = fs.open("/file.txt");
    ASSERT_TRUE(open_res.has_value());

    auto write_res = fs.write(open_res.value(), data);
    ASSERT_TRUE(write_res.has_value()) << toString(write_res.error());

    auto seek_res = fs.seek(open_res.value(), 0);
    ASSERT_TRUE(seek_res.has_value());

    auto read_res = fs.read(open_res.value(), data.size());
    ASSERT_TRUE(read_res.has_value());
    ASSERT_EQ(data, read_res.value());
}
