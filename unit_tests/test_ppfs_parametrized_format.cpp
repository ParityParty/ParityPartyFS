...
INSTANTIATE_TEST_SUITE_P(PpFSFormat, PpFSParametrizedTest,
...
TEST_P(PpFSParametrizedTest, Format_ProducesValidSuperblock)
{
    ...
    ASSERT_EQ(sb.ecc_type, config.ecc_type) << "ECC type mismatch for " << GetParam().test_name;
    ...
    ASSERT_GT(sb.total_blocks, 0) << "Total blocks should be > 0 for " << GetParam().test_name;
    ASSERT_GT(sb.total_inodes, 0) << "Total inodes should be > 0 for " << GetParam().test_name;
