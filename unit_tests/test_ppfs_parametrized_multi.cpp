...
TEST_P(PpFSParametrizedTest, OpenMultipleFiles)
{
    ...
    for (size_t i = 0; i < files.size(); ++i) {
        ...
        file_data.push_back(createTestData(GetParam().block_size / 2, static_cast<uint8_t>(0x10 + i)));
    }
...
TEST_P(PpFSParametrizedTest, ErrorCorrection_MultipleFiles)
{
    ...
    for (size_t i = 0; i < files.size(); ++i) {
        ...
        file_data.push_back(createTestData(GetParam().block_size / 2, static_cast<uint8_t>(0xAA + i)));
